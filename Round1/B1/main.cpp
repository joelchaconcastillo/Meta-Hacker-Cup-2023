...
bool helper(vector<long long> &factors, int item, long long cP, long long cS, vector<long long> &res){
	...
	for(int i = item; i < factors.size(); i++){
	   ...
	   hassol = helper(factors, i, cP/factors[i], cS-factors[i], res);
	   if(hassol) break;
	   res.pop_back();
	}
	return memo[item][cS]=hassol;
}
